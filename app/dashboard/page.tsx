  ...
  useEffect(() => {
    ...
    if (user) {
      const fetchData = async () => {
        try {
          const [eventsRes, rsvpsRes, usersRes] = await Promise.all([
            axios.get(`${API_URL}/events`),
            axios.get(`${API_URL}/rsvps`),
            axios.get(`${API_URL}/users`),
          ]);
          setAllEvents(eventsRes.data);
          setAllRSVPs(rsvpsRes.data);
  ...
  const handleDeleteEvent = async (eventId: string) => {
    if (window.confirm("Are you sure you want to delete this event? This action cannot be undone.")) {
      try {
        ...
        setAllRSVPs(allRSVPs.filter((rsvp) => rsvp.eventId !== eventId));
      } catch (error) {
              ...
              <CardContent>
                <div className="space-y-4">
                  {filteredUsers.map((userData) => {
                    const userEvents = allEvents.filter((event) => event.organizerId === userData._id)
